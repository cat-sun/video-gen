...
class MultiControlVaceAdapter(nn.Module):
    """Fuse normal/depth VACE latents while keeping the downstream VACE context shape."""
    ...
    def forward(self, normal_latents, depth_latents):
        gate = torch.softmax(self.gate_logits.to(dtype=normal_latents.dtype), dim=0)
        fused = gate[0] * normal_latents + gate[1] * depth_latents
        return fused + self.zero_conv(fused)
    ...
    @classmethod
    def from_pretrained(cls, input_dir, latent_channels):
        model = cls(latent_channels)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Fuse normal_file_path/control_file_path and depth_file_path VAE latents with a learnable VACE adapter.",
    )
    parser.add_argument(
        "--weighting_scheme",
        type=str,
...
def main():
    ...
    if args.enable_multi_control_adapter:
        multi_control_adapter = MultiControlVaceAdapter(latent_channels=vae.latent_channels * 2).to(weight_dtype)
                ...
                with torch.no_grad():
                    ...
                    if inpaint_flag or (control_pixel_values == -1).all():
                        ...
                        if multi_control_adapter is not None:
                            ...
                            vace_latents = multi_control_adapter(normal_vace_latents, depth_vace_latents)
                            vace_latents = [vace_latents[i] for i in range(vace_latents.size(0))]
                        else:
